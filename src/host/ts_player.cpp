...
void Player::run()
{
    ...
    //
    // It still asks for SCHED_FIFO, because the failure it has to survive is being descheduled
    // behind an unrelated CPU-bound process for longer than the lead. This is best-effort: without
    // an RLIMIT_RTPRIO allowance or CAP_SYS_NICE the call fails, and the thread simply runs at
    // normal priority -- which is fine at the default lead, and is why nothing is reported here.
    // (macOS joins the device's os_workgroup instead; Linux has no equivalent.)
    sched_param param{};
